...
Adds Amazon Athena connectivity as an installable connector extension.

This connector is listed in the public Irodori extension marketplace.

## Connector

- Extension ID: `irodori.athena`
- Engine ID: `athena`
- Wire: `lakehouse`
- Default port: `443`
...
The Rust code keeps native ABI exports in `src/lib.rs`, shared buffer/JSON helpers in `src/abi.rs`, and Athena SDK behavior in `src/driver.rs`.
...
| `connect` | Validates AWS credentials and the configured Athena workgroup. |
| `query` | Runs SQL through Athena `StartQueryExecution` and fetches result pages. |
| `metadata` | Reads table and column metadata through `information_schema.columns`. |
